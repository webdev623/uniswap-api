...
export interface MappedDetailedPair extends Pair {
  ...
  previous24hToken0Price: BigNumber
  previous24hToken1Price: BigNumber
}

export async function getTopPairs(pairLimit = 0): Promise<MappedDetailedPair[]> {
  const queryPairLimit = pairLimit > 0 ? pairLimit : TOP_PAIR_LIMIT
  ...
  const yesterdayVolumeIndex =
    pairVolumes?.reduce<{ [pairId: string]: {
      ...
      memo[pair.id] = {
        ...
        token0Price: new BigNumber(pair.token0Price),
        token1Price: new BigNumber(pair.token1Price)
      }
      return memo
    }, {}) ?? {}
    ...
    pairs?.map(
      (pair): MappedDetailedPair => {
        ...
        return {
          ...
          previous24hToken0Price: yesterday?.token0Price,
          previous24hToken1Price: yesterday?.token1Price
        }
      }
    ) ?? []
  )
...
export function getChangeInPercentage(val1: BigNumber, val2: BigNumber): BigNumber {
  if (val1.eq(0) || val2.eq(0)) {
    return new BigNumber(0)
  }
  
  const delta: BigNumber = val2.minus(val1)
  const raisedPercent = delta.multipliedBy(100).dividedBy(val1)
